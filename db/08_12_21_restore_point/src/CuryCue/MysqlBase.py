...
class MysqlBase:
        ...
        def ConnectDb(self):
                if self.conn is None or self.conn.is_connected() is False:
        ...
        def DisconnectDb(self):
                if self.conn.is_connected():
                        ...
                        ui.status="Подключение к mysql закрыто"
        ...
        def insertIntoTable(self, inputQuery):
                ...
                myFieldsTemplate=", ".join(['%s' for field in inputQuery.fieldsData])
        ...
        def executeUpdateQuery(self, query, arguments):
                ...
                try:
                        ...
                        return (False, "Что-то пошло не так: {}".format(err))
