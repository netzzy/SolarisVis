...
#		BaseClassesLocation.SceneToggleObject.__init__(self)
	...
	def GetActivePresetOutputs(self):
		...
		for item in self.ActiveList:
			...
			if self.ownerComp.op(item).op("LASER_MASER_OUT") is not None:
				...
				haveOutput=True
			if haveOutput and self.ownerComp.op(item).par.Activefade>0:
				haveouts.append(self.ownerComp.op(item).name)

			
		# return [" ".join(tops)]
		return [" ".join(tops), " ".join(sops), " ".join(haveouts)]
	...
	def Update(self):
		...
		[tops, mysops, haveouts]=self.GetActivePresetOutputs()



		# print (haveouts)
		# [tops]=self.GetActivePresetTops()
		# for myModule in [tops, mysops]:
		# 	print (myModule)

		# !!!!!!!!!!!!!!!!
		op.contentbar.Pushactive(haveouts)
		# !!!!!!!!!!!!!!!!
